...
*----------------------------------------------------------------------*
  ...
  IF sy-subrc <> 0 OR ls_cfg-active <> abap_true.
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<ky>).
      ...
      " CHANGE: without a FAILED entry RAP downgrades the error to a
      "         warning and Fiori shows the "proceed anyway" dialog
...
*----------------------------------------------------------------------*
  ...
  LOOP AT lt_failed-tour ASSIGNING FIELD-SYMBOL(<fail>).
    ...
    " CHANGE: propagate the failure so the message stays an error
...
*----------------------------------------------------------------------*
  LOOP AT lt_tours INTO DATA(ls_tour).
    ...
    IF lv_auth_error IS NOT INITIAL.
      ...
      " CHANGE: nothing was sent for this tour — mark the action failed
...
*          difference — distinctive enough that it cannot collide with a
*          data value.
...
*----------------------------------------------------------------------*
* CHANGE — nothing went out for this tour
*          RAP downgrades an error message in REPORTED to a warning when
*          the key is not also in FAILED. That is what produced the
*          "Warnungen / Tour an BMS freigeben / Abbrechen" dialog even
*          though every message above passes severity-error.
*          Marking the key failed keeps the messages at error severity
*          and stops Fiori offering to run the action again.
*----------------------------------------------------------------------*
    IF lv_count_ok = 0.
      APPEND VALUE #( %tky = ls_tour-%tky ) TO failed-tour.
      CONTINUE.
