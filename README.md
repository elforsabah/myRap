METHOD terminateservice.
    DATA: lt_unassign_keys  TYPE TABLE FOR ACTION IMPORT /plce/r_pdtour\\serviceassignment~unassign,
          lt_service_delete TYPE TABLE FOR DELETE /plce/r_pdservice, " <--- Used for hard deletion
    ...
    LOOP AT lt_services ASSIGNING FIELD-SYMBOL(<ls_service>).
      ...
      IF lt_return IS INITIAL.
        ...
        " A. Prepare Deletion (The Nuclear Option)
        APPEND VALUE #( %tky = <ls_service>-%tky ) TO lt_service_delete.
        ...
        " C. Success Message (with leading zeros removed)
    ...
    " -------------------------------------------------------------------------
    " 4. Bulk Modify RAP Entities (HARD DELETION)
    " -------------------------------------------------------------------------
    IF lt_service_delete IS NOT INITIAL.
      MODIFY ENTITIES OF /plce/r_pdservice IN LOCAL MODE
        ENTITY service
          DELETE FROM lt_service_delete.
    ENDIF.
    ...
    " Note: Deleted services will naturally return no data here, causing Fiori to instantly drop the row!
    READ ENTITIES OF /plce/r_pdservice IN LOCAL MODE
      ENTITY service
