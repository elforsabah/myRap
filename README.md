...
*&
*& Payload follows the live spec (InputOrder, openapi 3.0.4):
*&   - status is "ok" / "cancelled"
*&   - movementType is new | change | collect
*&   - containerTypeName only, containerTypeNumber no longer exists
*&   - no order / executionDate / executionTime / positions / contractRelated
*&   - additionalProperties: false — any extra field is rejected
*&---------------------------------------------------------------------*
REPORT zz_test_http1.

PARAMETERS:
  p_order  TYPE char20   DEFAULT 'TEST0001'   OBLIGATORY,
  p_date   TYPE d        DEFAULT sy-datum     OBLIGATORY,
  p_move   TYPE char10   DEFAULT 'change'     OBLIGATORY,
  p_ctype  TYPE char60   DEFAULT 'Absetzcontainer 10 cbm' OBLIGATORY,
  p_cold   TYPE char20   DEFAULT '802487',
  p_cnew   TYPE char20   DEFAULT '802512',
  p_tour   TYPE char20   DEFAULT '1229',
...
*----------------------------------------------------------------------*
  ...
  WRITE: / '=== Konfiguration                    ==='.
  WRITE: / '========================================'.
  WRITE: / 'Destination :', ls_cfg-bms_destination.
...
*----------------------------------------------------------------------*
  ...
  zcl_wr_pd_tour_helper=>get_bms_bearer_token(
    EXPORTING
      iv_destination = ls_cfg-bms_destination
...
*----------------------------------------------------------------------*
  ...
  cl_http_client=>create_by_destination(
    EXPORTING
      destination              = ls_cfg-bms_destination
    IMPORTING
      client                   = lo_http
    EXCEPTIONS
  ...
  ELSE.
    ...
    cl_http_utility=>set_request_uri(
      ...
      path    = '/BmsApiSapTest/api/user/healthCheck' ).
...
*----------------------------------------------------------------------*
  ...
  "   change  — both
  DATA(lv_cold) = CONV string( p_cold ).
  DATA(lv_cnew) = CONV string( p_cnew ).
  ...
  DATA(lv_cont) =
    `{` &&
      ...
      `"movementType":"`       && condense( CONV string( p_move ) )  && `",` &&
