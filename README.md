    ...
    return PageController.extend("com.prologa.zwrweighbrigefinal.ext.main.Main", {
        ...
        onStepActivate: function (oEvent) {
            ...
            if (sId === "stepK") {
                ...
                if (!sIndicator) {
                    // Skip if indicator has no value
                    this.oWizard.validateStep(oStep);
                    this.oWizard.nextStep();
                    return;
                } else {
                    // Focus input if step is active
...
.headText1 {
  font-size: 1rem;     /* adjust until it fits */
  font-weight: bold;
  white-space: nowrap;   /* keep number + KG together */

}

.tightWeights .sapMVBoxItem {
  margin-top: 0 !important;
  margin-bottom: 0 !important;
}

.hideStepK .sapMWizardProgressNavStep[data-step="1"] { /* Assuming stepK is the 3rd step (0-based index) */
