...
*         additionalProperties: false, so every component here must
*         exist in the spec and nothing may be added.
*----------------------------------------------------------------------*
  TYPES:
...
*----------------------------------------------------------------------*
      ...
      " Safety net: if the serialiser emits an empty object for an omitted
      " contact, strip it — {} would fail the required-field validation.
