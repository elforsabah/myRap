...
CLEAR: ev_vbeln, et_return[].
  ...
  IF sy-subrc <> 0.
    APPEND VALUE bapiret2( type = 'E' id = '00' number = '001'
      message = |Vertragsnummer { iv_contract } ist ungültig.| ) TO et_return.
    SET LOCALE LANGUAGE lv_old_langu.
    RETURN.
  ENDIF.

  "Validate it is actually a contract (VBTYP = 'G')
  IF ls_vbak-vbtyp <> 'G'.
    APPEND VALUE bapiret2( type = 'E' id = '00' number = '001'
      message = |Vertragsnummer { iv_contract } ist ungültig (kein Vertrag).| ) TO et_return.
    SET LOCALE LANGUAGE lv_old_langu.
    RETURN.
  ENDIF.
  ...
  IF sy-subrc <> 0 OR lv_auart IS INITIAL.
    lv_auart = iv_auart.
    APPEND VALUE bapiret2( type = 'W' id = '00' number = '001'
      message = |Keine Folgebelegart in TVAK für { ls_vbak-auart } gefunden, verwende { iv_auart }.| ) TO et_return.
  ENDIF.

  "3) Contract items for mapping (POSNR/MATNR/UoM)
  ...
  "4) Aggregate positions by contract POSNR (prevents duplicates / M_/011)
  ...
  "5) Build BAPI (CREATEFROMDATA1) structures
