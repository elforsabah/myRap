...
*          The unique sorted key also deduplicates.
...
*----------------------------------------------------------------------*
  LOOP AT lt_tours INTO DATA(ls_tour).
    ...
    LOOP AT lt_asgmts INTO DATA(ls_asgmt)
      ...
      " "S" does. That is what caused the intermittent
      " "Error converting value \"S\" to type InputMovementType".
      ...
      "   "The ContainerNumberNew field is required for the movement type 'S'."
      ...
      " Fail here with a readable message instead of letting BMS reject it.
      ...
      " No BINARY SEARCH — that addition is only allowed on standard tables;
      " a SORTED table searches its leading key field automatically.
      ...
      " "order" is a required non-empty string in BMS —
      "   {"errors":{"order":["The order field is required."]}}
      " ASP.NET [Required] rejects "" as well as a missing property, so the
      " chain must end on something that is always populated.
      ...
      " an empty string fail, so the chain falls back all the way to the
      " tour start date and the service is skipped if even that is missing.
...
*          Required.Always, producing
*            {"errors":{"customer":["Required property 'customer' not found"]}}
*            {"errors":{"estimatedDuration":["Required property ... not found"]}}
*          An empty string is a PRESENT property; an omitted field is not.
*
*          The 20 blind REPLACE statements stay gone — the BMS API binds
*          JSON case-insensitively (its own Swagger sample is camelCase).
*          One targeted REPLACE remains for the single genuine name
*          difference.
*----------------------------------------------------------------------*
