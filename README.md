...
**Extended behaviour Definition**

extension implementation in class zbp_zpd_r_service unique;

extend behavior for Service
{
  action ( precheck, features : instance ) changeServiceType parameter ZZPD_D_ServiceTypeMap result [0..*] $self;
//  action doAction01 parameter ZZPD_D_ServiceTypeMap { default function GetDefaultsForServType };
  side effects // line update after action excuting
  {
    action changeServiceType affects field ServiceType;
  }
...
{
//   determination calculateServiceTask on modify { field TaskType; }
}
...
association [0..1] to ZI_WR_EWA_ORDER_OBJECT as _ewa_order_object on  $projection.ServiceUUID = _ewa_order_object.PdServiceUuid 
 {
    _ewa_order_object.point_of_origin as point_of_origin
}

**the view itself that extended**
...
**A projection of the root service**
