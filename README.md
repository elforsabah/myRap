METHOD touranBMSfreigeben.
  ...
  "=======================================================================
  " STEP 1 — Read the tours that the dispatcher selected
  "=======================================================================
  ...
  "           (link table: gives ServiceUUID + sequence order)
  "=======================================================================
  READ ENTITIES OF /PLCE/R_PDTour IN LOCAL MODE
  ...
  "=======================================================================
  " STEP 3 — Read service headers from PROLOGA
  "           ReferenceInternalId = POBJNR in EWA_ORDER_OBJECT (join key)
  "           Action              = movement type (01-05)
  "           ServiceWindow*      = time frame fields
  "=======================================================================
  READ ENTITIES OF /PLCE/R_PDTourServiceAsgmt IN LOCAL MODE
    ENTITY ServiceAssignment BY \_Service
      FIELDS ( ServiceUUID
               ReferenceId
               ReferenceInternalId
               Action
               ...
               AdditionalText
               FunctionalLocation )
      WITH CORRESPONDING #( lt_asgmts )
    RESULT DATA(lt_services).

  "=======================================================================
  " STEP 4 — Bulk SELECT from EWA_ORDER_OBJECT
  "           Contains: order number, container, partners, waste key,
  "           notes, planned time, document number, express flag etc.
  "           Joined via POBJNR = ReferenceInternalId from PROLOGA
  "=======================================================================
  DATA lt_pobjnr TYPE RANGE OF j_objnr.
  lt_pobjnr = VALUE #(
    FOR ls IN lt_services
    WHERE ( ReferenceInternalId IS NOT INITIAL )
    ( sign   = 'I'
      option = 'EQ'
      low    = ls-ReferenceInternalId ) ).
  ...
  "           Collects customer (KUNNR), carrier (TRANSPORTER),
  "           recycler (DISPOSER) and goods-recipient (KUNWE) addresses
  ...
  LOOP AT lt_tours INTO DATA(ls_tour).
    ...
    "     Maintained by the basis/config team in SM30 on ZTOUR_BMS_KOLONNE.
    "     If no entry exists the order is sent without a team assignment
    "     (valid per interface doc — BMS leaves it unassigned).
    "-----------------------------------------------------------------------
    ...
    LOOP AT lt_tour_asgmts INTO DATA(ls_asgmt).
      ...
      " Service header for this assignment
      DATA(ls_svc) = VALUE /plce/r_pdservice(
                       OPTIONAL
                       ( lt_services[ ServiceUUID = ls_asgmt-ServiceUUID ] ) ).
      CHECK ls_svc IS NOT INITIAL.

      " Corresponding EWA order row
      DATA(ls_ewa) = VALUE ty_ewaobj(
                       OPTIONAL
                       ( lt_ewaobj[ pobjnr = ls_svc-ReferenceInternalId ] ) ).
      ...
      "     Confirmed action keys from /PLCE/CPDACTIONT:
      "       01 Stellen  → S    02 Holen    → H
      "       03 Tauschen → T    04 Ersetzen → T
      "       05 Umleeren → T
      "     (Sofortbefüllung → F not yet in action table; add when created)
      ...
      "     BEH_TYPE_NEW = container type, BEH_ANZAHL = count
      "     customerOwned: extend EWA_ORDER_OBJECT with ZZ field if needed
      "---------------------------------------------------------------------
      ...
      "     ReferenceId = SD sales order number
      "---------------------------------------------------------------------
      DATA lt_positions TYPE tt_positions.
      IF ls_svc-ReferenceId IS NOT INITIAL.
        SELECT vbeln, posnr, matnr, arktx, kwmeng, meins, netpr
          FROM vbap
          WHERE vbeln = @ls_svc-ReferenceId
      ...
      "---------------------------------------------------------------------
      " 6e  LEISTUNGSZEIT — derived from PROLOGA time window fields.
      "     Falls back to EWA PLANNED_TIME if PROLOGA window is empty.
      "     Produces the text patterns documented in the old interface:
      "     "14:00 bis 20:00" / "ab 16:00" / "bis 20:00" / "15:00"
      "---------------------------------------------------------------------
      DATA(lv_exec_time) = COND string(
        WHEN ls_svc-ServiceWindowStartTime IS NOT INITIAL
         AND ls_svc-ServiceWindowEndTime   IS NOT INITIAL
         AND ls_svc-ServiceWindowStartTime <> ls_svc-ServiceWindowEndTime
        THEN |{ ls_svc-ServiceWindowStartTime TIME = USER } bis | &&
             |{ ls_svc-ServiceWindowEndTime   TIME = USER }|
        WHEN ls_svc-ServiceWindowStartTime IS NOT INITIAL
         AND ls_svc-ServiceWindowEndTime   IS NOT INITIAL
        THEN |{ ls_svc-ServiceWindowStartTime TIME = USER }|
        WHEN ls_svc-ServiceWindowStartTime IS NOT INITIAL
        THEN |ab { ls_svc-ServiceWindowStartTime TIME = USER }|
        WHEN ls_svc-ServiceWindowEndTime IS NOT INITIAL
        THEN |bis { ls_svc-ServiceWindowEndTime TIME = USER }|
        WHEN ls_ewa-planned_time IS NOT INITIAL
        THEN |{ ls_ewa-planned_time TIME = USER }|
        ELSE CONV string( ls_svc-ServiceWindow ) ).

      "---------------------------------------------------------------------
      " 6f  signatureRequired
      "     Scans both ORDERTXT (EWA) and AdditionalText (PROLOGA) for
      "     the substring UTERSCHR (per old interface doc logic)
      "---------------------------------------------------------------------
      DATA(lv_sig_req) = xsdbool(
        to_upper( ls_ewa-ordertxt )       CS 'UTERSCHR' OR
        to_upper( ls_svc-AdditionalText ) CS 'UTERSCHR' ).

      "---------------------------------------------------------------------
      " 6g  Notes enrichment (all rules from the old interface doc)
      "     Base: ORDERTXT from EWA_ORDER_OBJECT
      "     Appended in order: KVV flag, Bestellnummer, Express flag,
      ...
      "     The real AVV code is in /WATP/AVVCODE but intentionally swapped
      "     so drivers see the more meaningful article on their tablet
      "---------------------------------------------------------------------
      ...
      "---------------------------------------------------------------------
      " 6j  Duration: PLANNED_DURT is stored as TIMS (HHMMSS) → convert
      "     to integer minutes for the BMS estimatedDuration field
      "---------------------------------------------------------------------
      ...
      "---------------------------------------------------------------------
      " 6k  Assemble the full BMS JSON payload
      "---------------------------------------------------------------------
      DATA(ls_order) = VALUE ty_bms_order(
        ...
        planned_date = COND string(
          WHEN ls_svc-EarliestDate IS NOT INITIAL
          THEN |{ ls_svc-EarliestDate DATE = ISO }T00:00:00.000Z|
          ELSE |{ ls_ewa-zz_order_date DATE = ISO }T00:00:00.000Z| )

        execution_date = COND string(
          WHEN ls_svc-RequestedDate IS NOT INITIAL
          THEN |{ ls_svc-RequestedDate DATE = ISO }T00:00:00.000Z|
        ...
        execution_time_frame_end = COND string(
          WHEN ls_svc-ServiceWindowEndTime IS NOT INITIAL
          THEN |{ ls_svc-ServiceWindowEndTime TIME = USER }| )
        ...
        special_notes = COND string(
          ...
          THEN |EXPRESSAUFTRAG| &&
               COND #( WHEN ls_svc-AdditionalText IS NOT INITIAL
                        THEN | - | && ls_svc-AdditionalText )
          ELSE ls_svc-AdditionalText )
      ...
      "=======================================================================
      " STEP 7 — Serialize to camelCase JSON
      "=======================================================================
      ...
      "=======================================================================
      " STEP 9 — Evaluate HTTP response and report back to the UI
      "=======================================================================
      IF lv_rc = 201.
