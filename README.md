...
METHOD identifycard.
  ...
  LOOP AT keys ASSIGNING FIELD-SYMBOL(<ls_key>).
    ...
    SELECT SINGLE @abap_true FROM vbak
      ...
      INTO @DATA(lv_exists).
    IF sy-subrc = 0.
      " ✓ Valid: set Vbeln and report success (modify server-side to avoid client PATCH/lock error)
      MODIFY ENTITIES OF zi_wr_weighingsession IN LOCAL MODE
        ENTITY WeighingSession
        UPDATE FIELDS ( vbeln ) // Optionally add step1ok if still needed
        WITH VALUE #( ( %tky = <ls_key>-%tky vbeln = <ls_key>-%param-vbeln ) ). // Optionally step1ok = abap_true
...
//  Keys must be readonly in strict draft
  field (readonly) Sessionid;  // Removed Vbeln (no longer key)
...
validation validateStep1 on save { field Vbeln, Sessionid; } // Identification // Keep, but Vbeln now non-key
                  ...
                  <f:FormElement id="step1FormElementInput" label="Please Enter your Contract ID">
                    <f:fields>
                      <Input id="step1InputContract"
                             value="" // CHANGED: Unbind value (no {Vbeln}) to avoid client-side model update/PATCH
                             width="100%"
                             maxLength="10"
                             required="true"
