    ...
    LOOP AT lt_tours INTO DATA(ls_tour).
      ...
      LOOP AT lt_asgmts INTO DATA(ls_asgmt)
        ...
        IF sy-subrc <> 0 OR lv_movement_type IS INITIAL.
          " Fallback to S (Stellen) if not configured — warn dispatcher
          lv_movement_type = 'S'.
          APPEND VALUE #(
            %tky = ls_tour-%tky
            %msg = new_message(
        ...
        "    When beh_type_new is initial, a minimal fallback entry is sent
        "    using the configured movement type from ZWRTWAALAPRCTP so that
        "    BMS min-1-element validation passes with a valid enum value.
        "    No fallback container is appended if movement type is also unknown
        "    to avoid sending an invalid OptiAwiMovementType value to BMS.
        ...
        ELSEIF lv_movement_type IS NOT INITIAL.
          " Minimal fallback — no container type known but movement type is set
          APPEND VALUE #(
            quantity       = 1
            movement_type  = lv_movement_type
            customer_owned = abap_false
        ...
        DATA(lv_order_ref) = COND string(
          ...
          ELSE                                      ls_svc-reference_id ).

        " 6m OrderNumber — smaufnr preferred; falls back to lv_order_ref
        "    when smaufnr is initial to avoid sending a blank string that
        "    BMS rejects as a required field
        ...
        DATA(lv_order_number) = COND string(
          ...
          THEN lv_smaufnr_clean
          ELSE lv_order_ref ).

        " 6n Trimmed BP number strings
        "    ALPHA = OUT removes leading zeros; condense removes trailing
        "    spaces that CHAR-typed partner fields carry when shorter than
