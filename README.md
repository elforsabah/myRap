METHOD createlanf.
  ...
  DATA:
    ls_header_in     TYPE bapisdhead,           " <-- from SE37
    lt_items_in      TYPE tt_bapiitemin,        " <-- from SE37
  ...
  "-------------------------
  DATA(lv_auart) = 'DR'.  " <-- your LANF doc type
...
METHOD map_positions_to_contract.
  ...
  LOOP AT it_positions ASSIGNING FIELD-SYMBOL(<pos>) WHERE menge > 0.
    ...
    READ TABLE lt_contract_pos ASSIGNING FIELD-SYMBOL(<c>)
      WITH KEY matnr = lv_matnr.
    IF sy-subrc <> 0.
      APPEND VALUE bapiret2(
        type   = 'E' id = '00' number = '001'
        message = |No position for MATNR { lv_matnr } in contract { iv_contract_vbeln }|
      ) TO ct_return.
      CONTINUE.
    ENDIF.
