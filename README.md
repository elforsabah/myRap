METHOD touranbmsfreigeben.
    ...
    LOOP AT lt_tours INTO DATA(ls_tour).
      ...
      LOOP AT lt_asgmts INTO DATA(ls_asgmt)
        ...
        "    If no container type is known a minimal entry is sent so that
        "    BMS min-1-element validation passes
        ...
        DATA(ls_order) = VALUE ty_bms_order(
          ...
          recycler = VALUE #(
            number        = COND #(
                              ...
                              THEN |{ ls_ewa-disposer ALPHA = OUT }|
                              ELSE |{ lv_def_recycler ALPHA = OUT }| )
          ...
          carrier = VALUE #(
            number        = COND #(
                              ...
                              THEN |{ ls_ewa-transporter ALPHA = OUT }|
                              ELSE |{ lv_def_carrier ALPHA = OUT }| )
        ...
        "          /UI2/CL_JSON writes null for initial numeric fields —
        "          replace itemPrice:null with 0 to pass BMS validation
        "          collectiveConsignmentNoteNumber differs from ABAP naming
        "=====================================================================
