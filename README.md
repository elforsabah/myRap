METHOD touranBMSfreigeben.
  ...
  "           FILTER replaced with LOOP AT ... WHERE to avoid the
  "           "SORTED or HASHED key required" error — READ ENTITIES
  "           returns standard tables which FILTER cannot work on
  "=======================================================================
  LOOP AT lt_tours INTO DATA(ls_tour).
    ...
    "-----------------------------------------------------------------------
    " Loop directly over lt_asgmts filtered by TourUUID — avoids FILTER
    " which requires a SORTED/HASHED table. LOOP AT ... WHERE works on
    " any table type and is equally efficient after the SORT in Step 2.
    "-----------------------------------------------------------------------
