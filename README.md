...
*----------------------------------------------------------------------*
  LOOP AT lt_tours INTO DATA(ls_tour).
    ...
    LOOP AT lt_asgmts INTO DATA(ls_asgmt)
      ...
      " overview ("Containertausch", "Leerung", "Abholung"). It and
      " internalRemark are required by the running API and [Required]
      " rejects empty strings, so both always carry content.
